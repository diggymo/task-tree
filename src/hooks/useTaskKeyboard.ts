...
export function useTaskKeyboard({
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLTextAreaElement>, taskId: string) => {
            ...
            const updateParent = (
              ...
              return {
                ...
                children: node.children.map(
                  (child) => updateParent(child) as TaskNode,
                ),
              };
            };
      ...
      if (e.key === 'Backspace' && e.currentTarget.value === '' && parent) {
        e.preventDefault();
        const siblingIndex =
          index > 0 ? index - 1 : parent.children.length > 1 ? 1 : -1;
        const nextFocusId =
          siblingIndex >= 0 ? parent.children[siblingIndex]?.id : parent.id;
